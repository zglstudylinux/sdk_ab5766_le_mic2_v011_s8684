...
- `BSP_LED_EN` 决定 LED 代码是否参与编译；`xcfg_cb.bled_disp_en` 决定运行时是否允许蓝灯操作。蓝灯禁用时，`bled_set_on()`、`bled_set_off()` 与 `led_scan()` 会直接返回，避免驱动未初始化或未确认用途的 GPIO。
- 配置界面的 PA0 仅是默认选项，不代表板级 LED 一定接在 PA0。启用蓝灯前必须确认实际 GPIO、有效电平、独立/共脚拓扑和该引脚没有被其他外设占用。
- 若蓝、红灯都启用且配置为同一 IO，代码会使用数字/模拟模式与电平复用一个 IO 驱动两灯；当前 `BSP_CHARGE_EN=0`，不应把红色充电指示灯当作已启用功能。
- `led_bt_idle()` 与 `led_bt_connected()` 会选择“未连接/配对”和“已连接”的灯光状态；状态参数可来自 `xcfg_cb`，否则使用代码默认值。只要存在任意一条无线连接即为“已连接”；最后一条连接断开才回到未连接状态，连接失败没有独立灯效。
- 默认配对图样的源代码注释标为“亮 300 ms、灭 450 ms”，但 `led_scan()` 实际按 50 ms 节拍处理 `led_cfg_pairing={0x00, 0x0f, 2, 2}`：约亮 400 ms、灭 500 ms、周期约 900 ms；已连接状态为蓝灯常亮。
...
## 10. 音频算法专题：从“能听到声音”到可复现实测

第 9 节只用于建立总览，不能据此确认某个算法已经运行或通过。当前默认链路的完整源码调用关系、文件/函数/行号、运行时门禁、预编译库边界和逐项测试方法见：[AB5766_LE_Mic_音频算法调用链与测试快速入门.md](AB5766_LE_Mic_音频算法调用链与测试快速入门.md)。

开始硬件算法试验前，先建立 E0～E3 证据：编译期与运行时配置（E0）、真实源码调用关系（E1）、当次 map/固件/effect/xcfg 产物（E2）、双板音频与仪器原始记录（E3）。统一模板见：[../plan/AB5766_LE_Mic_算法测试执行记录.md](../plan/AB5766_LE_Mic_算法测试执行记录.md)。

特别注意：连接成功后 `wireless_emit_notice()` 才按角色初始化 `mic_emit_init()` 或 `adapter_init()` 并打开 `sys_cb.mic_alg_en`；因此“宏为 1”“map 有符号”“已有声音”都不是算法生效的充分证据。先完成 `ALG-BASE`，再按 Soft Gain、Local PACC、LC3S、PLC、双链路混音和输出稳定性顺序推进。
